<?php
require_once('class/class-liqpay.php');

defined( 'ABSPATH' ) || die();

add_action( 'wp', array( 'GFLiqPay', 'maybe_thankyou_page' ), 5 );

GFForms::include_payment_addon_framework();

class GFLiqPay extends GFPaymentAddOn {

    protected $_version = GF_LIQPAY_VERSION;
    protected $_min_gravityforms_version = '1.9.3';
    protected $_slug = 'gf_liqpay_gw';
    protected $_path = 'gf_liqpay_gw/liqpay.php';
    protected $_full_path = __FILE__;
    protected $_url = 'http://www.gravityforms.com';
    protected $_title = 'Gravity Forms LiqPay';
    protected $_short_title = 'LiqPay';
    protected $_supports_callbacks = true;
    private $production_url = 'https://www.liqpay.com/cgi-bin/webscr/';
    private $sandbox_url = 'https://www.sandbox.liqpay.com/cgi-bin/webscr/';

    // Members plugin integration
    protected $_capabilities = array( 'gravityforms_liqpay', 'gravityforms_liqpay_uninstall' );

    // Permissions
    protected $_capabilities_settings_page = 'gravityforms_liqpay';
    protected $_capabilities_form_settings = 'gravityforms_liqpay';
    protected $_capabilities_uninstall = 'gravityforms_liqpay_uninstall';
    ...
    public function plugin_settings_fields() {
        $description = '
            ...
            '</p>
			<ul>
            <li>' . sprintf( esc_html__( 'Notification URL: %s', 'gravityformsliqpay' ), '<strong>' . esc_url( add_query_arg( 'page', 'gf_liqpay_ipn', get_bloginfo( 'url' ) . '/' ) ) . '</strong>' ) . '</li>' .
            '</ul>
				<br/>';
        ...
        return array(
            array(
                ...
                'fields'      => array(
                    array(
                        'name'    => 'gf_liqpay_configured',
                        'label'   => esc_html__( 'LiqPay', 'gravityformsliqpay' ),
                        'type'    => 'checkbox',
                        'choices' => array( array( 'label' => esc_html__( 'Confirm that you have configured your LiqPay account', 'gravityformsliqpay' ), 'name' => 'gf_liqpay_configured' ) )
                    ),
                    array(
                        'name'     => 'lp_prod_pub_key',
                        'label'    => esc_html__( 'LiqPay Prod. Public Key ', 'gravityformsliqpay' ),
                        'type'     => 'text',
                        'class'    => 'medium',
//                        'required' => true,
                        'tooltip'  => '<h6>' . esc_html__( 'LiqPay Production Public Key', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enter the LiqPay production public key.', 'gravityformsliqpay' )
                    ),
                    array(
                        'name'     => 'lp_prod_secret_key',
                        'label'    => esc_html__( 'LiqPay Prod. Secret Key ', 'gravityformsliqpay' ),
                        'type'     => 'text',
                        'class'    => 'medium',
//                        'required' => true,
                        'tooltip'  => '<h6>' . esc_html__( 'LiqPay Production Secret Key', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enter the LiqPay production secret key.', 'gravityformsliqpay' )
                    ),
                    array(
                        'name'     => 'lp_test_pub_key',
                        'label'    => esc_html__( 'LiqPay Test Public Key ', 'gravityformsliqpay' ),
                        'type'     => 'text',
                        'class'    => 'medium',
//                        'required' => true,
                        'tooltip'  => '<h6>' . esc_html__( 'LiqPay Test Public Key', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enter the LiqPay test public key.', 'gravityformsliqpay' )
                    ),
                    array(
                        'name'     => 'lp_test_secret_key',
                        'label'    => esc_html__( 'LiqPay Test Secret Key ', 'gravityformsliqpay' ),
                        'type'     => 'text',
                        'class'    => 'medium',
//                        'required' => true,
                        'tooltip'  => '<h6>' . esc_html__( 'LiqPay Test Secret Key', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enter the LiqPay test secret key.', 'gravityformsliqpay' )
                    ),
                    array(
                        'type' => 'save',
                        'messages' => array(
                            'success' => esc_html__( 'Settings have been updated.', 'gravityformsliqpay' )
                        ),
                    ),
    ...
    public function feed_list_no_item_message() {
        ...
        if ( ! rgar( $settings, 'gf_liqpay_configured' ) ) {
            return sprintf( esc_html__( 'To get started, please configure your %sLiqPay Settings%s!', 'gravityformsliqpay' ), '<a href="' . admin_url( 'admin.php?page=gf_settings&subview=' . $this->_slug ) . '">', '</a>' );
        } else {
    ...
    public function feed_settings_fields() {
        ...
        //--add LiqPay Email Address field
        $fields = array(
            array(
                'name'     => 'liqpayEmail',
                'label'    => esc_html__( 'LiqPay Email Address ', 'gravityformsliqpay' ),
                'type'     => 'text',
                'class'    => 'medium',
                'required' => true,
                'tooltip'  => '<h6>' . esc_html__( 'LiqPay Email Address', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enter the LiqPay email address where payment should be received.', 'gravityformsliqpay' )
            ),
            array(
                'name'          => 'mode',
                'label'         => esc_html__( 'Mode', 'gravityformsliqpay' ),
                'type'          => 'radio',
                'choices'       => array(
                    array( 'id' => 'gf_liqpay_mode_production', 'label' => esc_html__( 'Production', 'gravityformsliqpay' ), 'value' => 'production' ),
                    array( 'id' => 'gf_liqpay_mode_test', 'label' => esc_html__( 'Test', 'gravityformsliqpay' ), 'value' => 'test' ),

                ),

                'horizontal'    => true,
                'default_value' => 'production',
                'tooltip'       => '<h6>' . esc_html__( 'Mode', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Select Production to receive live payments. Select Test for testing purposes when using the LiqPay development sandbox.', 'gravityformsliqpay' )
            ),
        );
        ...
        if ( $add_donation ) {
            ...
            $choices[] = array( 'label' => __( 'Donations', 'gravityformsliqpay' ), 'value' => 'donation' );
        }
        ...
        $fields = array(
            array(
                'name'     => 'imageURL',
                'label'    => esc_html__( 'Image URL', 'gravityformsliqpay' ),
                'type'     => 'text',
                'class'    => 'medium',
                'required' => false,
                'tooltip'  => '<h6>' . esc_html__( 'Image URL', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'This option allows you to enter the URL of the 150x50-pixel image displayed as your logo in the upper left corner of the LiqPay checkout pages. Default is your business name, if you have a LiqPay Business account or your email address, if you have LiqPay Premier or Personal account.', 'gravityformsliqpay' )
            ),
            array(
                'name'     => 'cancelUrl',
                'label'    => esc_html__( 'Cancel URL', 'gravityformsliqpay' ),
                'type'     => 'text',
                'class'    => 'medium',
                'required' => false,
                'tooltip'  => '<h6>' . esc_html__( 'Cancel URL', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enter the URL the user should be sent to should they cancel before completing their LiqPay payment.', 'gravityformsliqpay' )
            ),
            array(
                'name'    => 'options',
                'label'   => esc_html__( 'Options', 'gravityformsliqpay' ),
                'type'    => 'options',
                'tooltip' => '<h6>' . esc_html__( 'Options', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Turn on or off the available LiqPay checkout options.', 'gravityformsliqpay' ),
                'choices' => array(
                    array(
                        'label' => esc_html__( 'Do not prompt buyer to include a shipping address.', 'gravityformsliqpay' ),
                        'name'  => 'disableShipping',
                    ),
                    array(
                        'label' => esc_html__( 'Do not prompt buyer to include a note with payment.', 'gravityformsliqpay' ),
                        'name'  => 'disableNote',
                    ),
                ),
            ),
        ...
        if ( $this->get_setting( 'delayNotification' ) || ! $this->is_gravityforms_supported( '1.9.12' ) ) {
            $fields[] = array(
                'name'    => 'notifications',
                'label'   => esc_html__( 'Notifications', 'gravityformsliqpay' ),
                'type'    => 'notifications',
                'tooltip' => '<h6>' . esc_html__( 'Notifications', 'gravityformsliqpay' ) . '</h6>' . esc_html__( "Enable this option if you would like to only send out this form's notifications for the 'Form is submitted' event after payment has been received. Leaving this option disabled will send these notifications immediately after the form is submitted. Notifications which are configured for other events will not be affected by this option.", 'gravityformsliqpay' )
            );
        }
        ...
        if ( GFCommon::has_post_field( $form['fields'] ) ) {
            $post_settings = array(
                ...
                'label'   => esc_html__( 'Posts', 'gravityformsliqpay' ),
                'type'    => 'checkbox',
                'tooltip' => '<h6>' . esc_html__( 'Posts', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Enable this option if you would like to only create the post after payment has been received.', 'gravityformsliqpay' ),
                'choices' => array(
                    array( 'label' => esc_html__( 'Create post only when payment is received.', 'gravityformsliqpay' ), 'name' => 'delayPost' ),
                ),
            );

            if ( $this->get_setting( 'transactionType' ) == 'subscription' ) {
                $post_settings['choices'][] = array(
                    'label'    => esc_html__( 'Change post status when subscription is canceled.', 'gravityformsliqpay' ),
        ...
        //Adding custom settings for backwards compatibility with hook 'gform_liqpay_add_option_group'
        $fields[] = array(
            'name'  => 'custom_options',
            'label' => '',
            'type'  => 'custom',
        ...
        if ( $add_last_name ) {
            //add last name
            array_unshift( $billing_info['field_map'], array( 'name' => 'lastName', 'label' => esc_html__( 'Last Name', 'gravityformsliqpay' ), 'required' => false ) );
        }
        if ( $add_first_name ) {
            array_unshift( $billing_info['field_map'], array( 'name' => 'firstName', 'label' => esc_html__( 'First Name', 'gravityformsliqpay' ), 'required' => false ) );
        }
        $default_settings = parent::replace_field( 'billingInformation', $billing_info, $default_settings );
        //----------------------------------------------------------------------------------------------------

        //hide default display of setup fee, not used by LiqPay Standard
        $default_settings = parent::remove_field( 'setupFee', $default_settings );

        //--add trial period
        $trial_period     = array(
            'name'    => 'trialPeriod',
            'label'   => esc_html__( 'Trial Period', 'gravityformsliqpay' ),
            'type'    => 'trial_period',
            'hidden'  => ! $this->get_setting( 'trial_enabled' ),
            'tooltip' => '<h6>' . esc_html__( 'Trial Period', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Select the trial period length.', 'gravityformsliqpay' )
        );
        ...
        $recurring_retry  = array(
            'name'       => 'recurringRetry',
            'label'      => esc_html__( 'Recurring Retry', 'gravityformsliqpay' ),
            'type'       => 'checkbox',
            'horizontal' => true,
            'choices'    => array( array( 'label' => esc_html__( 'Try to bill again after failed attempt.', 'gravityformsliqpay' ), 'name' => 'recurringRetry', 'value' => '1' ) ),
            'tooltip'    => '<h6>' . esc_html__( 'Recurring Retry', 'gravityformsliqpay' ) . '</h6>' . esc_html__( 'Turn on or off whether to try to bill again after failed attempt.', 'gravityformsliqpay' )
        );
        ...
        /**
         * Filter through the feed settings fields for the Liqpay feed
         *
         * @param array $default_settings The Default feed settings
         * @param array $form The Form object to filter through
         */
        return apply_filters( 'gform_liqpay_feed_settings_fields', $default_settings, $form );
    }

    public function supported_billing_intervals() {

        $billing_cycles = array(
            'day'   => array( 'label' => esc_html__( 'day(s)', 'gravityformsliqpay' ), 'min' => 1, 'max' => 90 ),
            'week'  => array( 'label' => esc_html__( 'week(s)', 'gravityformsliqpay' ), 'min' => 1, 'max' => 52 ),
            'month' => array( 'label' => esc_html__( 'month(s)', 'gravityformsliqpay' ), 'min' => 1, 'max' => 24 ),
            'year'  => array( 'label' => esc_html__( 'year(s)', 'gravityformsliqpay' ), 'min' => 1, 'max' => 5 )
        );
    ...
    public function field_map_title() {
        return esc_html__( 'LiqPay Field', 'gravityformsliqpay' );
    }
    ...
    public function settings_options( $field, $echo = true ) {
        ...
        ob_start();
        do_action( 'gform_liqpay_action_fields', $this->get_current_feed(), $this->get_current_form() );
        $html .= ob_get_clean();
        //--------------------------------------------------------
    ...
    public function settings_custom( $field, $echo = true ) {
        ...
        ob_start();
        ?>
        <div id='gf_liqpay_custom_settings'>
            <?php
            do_action( 'gform_liqpay_add_option_group', $this->get_current_feed(), $this->get_current_form() );
            ?>
        </div>

        <script type='text/javascript'>
            jQuery(document).ready(function () {
                jQuery('#gf_liqpay_custom_settings label.left_header').css('margin-left', '-200px');
            });
        </script>
    ...
    public function settings_notifications( $field, $echo = true ) {
        $checkboxes = array(
            ...
            'choices' => array(
                array(
                    'label' => esc_html__( "Send notifications for the 'Form is submitted' event only when payment is received.", 'gravityformsliqpay' ),
        ...
        ob_start();
        ?>
        <ul id="gf_liqpay_notification_container" style="padding-left:20px; margin-top:10px; <?php echo $has_delayed_notifications ? '' : 'display:none;' ?>">
            <?php
            if ( ! empty( $form ) && is_array( $form['notifications'] ) ) {
                ...
                foreach ( $notifications as $notification ) {
                    ?>
                    <li class="gf_liqpay_notification">
                        ...
                        <label class="inline" for="gf_liqpay_selected_notifications"><?php echo $notification['name']; ?></label>
                    </li>
                    <?php
                }
            }
        ...
        <script type='text/javascript'>
            ...
            function ToggleNotifications() {
                ...
                var container = jQuery('#gf_liqpay_notification_container');
                var isChecked = jQuery('#delaynotification').is(':checked');

                if (isChecked) {
                    container.slideDown();
                    jQuery('.gf_liqpay_notification input').prop('checked', true);
                }
                else {
                    container.slideUp();
                    jQuery('.gf_liqpay_notification input').prop('checked', false);
                }
    ...
    public function checkbox_input_change_post_status( $choice, $attributes, $value, $tooltip ) {
        ...
        $dropdown_field = array(
            ...
            'choices'  => array(
                array( 'label' => '' ),
                array( 'label' => esc_html__( 'Mark Post as Draft', 'gravityformsliqpay' ), 'value' => 'draft' ),
                array( 'label' => esc_html__( 'Delete Post', 'gravityformsliqpay' ), 'value' => 'delete' ),
    ...
    public function save_feed_settings( $feed_id, $form_id, $settings ) {
        ...
        $feed['meta'] = $settings;
        $feed         = apply_filters( 'gform_liqpay_save_config', $feed );

        //call hook to validate custom settings/meta added using gform_liqpay_action_fields or gform_liqpay_add_option_group action hooks
        $is_validation_error = apply_filters( 'gform_liqpay_config_validation', false, $feed );
    ...
    public function check_ipn_request() {
        ...
        if ( empty( $can_verify_ipn ) ) {
            ...
            $url = 'https://www.sandbox.liqpay.com/cgi-bin/webscr/';
            $request  = new WP_Http();
            ...
            if ( ! is_wp_error( $response ) && rgar( $response, 'body' ) == 'INVALID' ) {
                ...
                wp_mail( get_bloginfo( 'admin_email' ), 'Immediate Action Required: SSL certificate is outdated', 'WARNING: Your web server does not currently support the SHA-2 SSL Certificate standard required by LiqPay.  <a href="https://devblog.liqpay.com/liqpay-ssl-certificate-changes/">For details see LiqPay\'s changeover announcement</a>. Please contact your web host to resolve this issue as soon as possible.' );
                $can_verify_ipn = 'no';
            }
        ...
        if ( $can_verify_ipn == 'no' ) {
            ...
            //display message
            echo '<div class="error"> <p><strong>WARNING:</strong> Your web server does not currently support the SHA-2 SSL Certificate standard required by LiqPay.  <a href="https://devblog.liqpay.com/liqpay-ssl-certificate-changes/">For details see LiqPay\'s changeover announcement</a>. Please contact your web host to resolve this issue as soon as possible. <a href="' . add_query_arg( array( 'dismiss_ipn_check' => 1 ) ) . '">Dismiss</a></p></div>';
    ...
    public function redirect_url( $feed, $submission_data, $form, $entry ) {
        ...
        //Don't process redirect url if request is a LiqPay return
        if ( ! rgempty( 'gf_liqpay_return', $_GET ) ) {
            return false;
        }
        ...
        $invoice_id = apply_filters( 'gform_liqpay_invoice', '', $form, $entry );
        ...
        //Set return mode to 2 (LiqPay will post info back to page). rm=1 seems to create lots of problems with the redirect back to the site. Defaulting it to 2.
        $return_mode = '2';
...
//        $return_url = '&return=' . urlencode( $this->return_url( $form['id'], $entry['id'] ) ) . "&rm={$return_mode}";
        ...
        //URL that will listen to notifications from LiqPay
        $ipn_url = urlencode( get_bloginfo( 'url' ) . '/?page=gf_liqpay_ipn' );

        $business_email = urlencode( trim( $feed['meta']['liqpayEmail'] ) );
        ...
        $query_string = gf_apply_filters( 'gform_liqpay_query', $form['id'], $query_string, $form, $entry, $feed, $submission_data );

        if ( ! $query_string ) {
            $this->log_debug( __METHOD__ . '(): NOT sending to LiqPay: The price is either zero or the gform_liqpay_query filter was used to remove the querystring that is sent to LiqPay.' );
...
//        $url .= $query_string;
        ...
        {
            $liqpay = new LiqPayForGravityForms($settings["lp_prod_pub_key"], $settings["lp_prod_secret_key"]);
        }
        if($feed['meta']['mode'] == 'test')
        {
            $liqpay = new LiqPayForGravityForms($settings["lp_test_pub_key"], $settings["lp_test_secret_key"]);
        }
        $html = $liqpay->cnb_form_raw(array(
            'action'         => 'pay',
            ...
            'order_id'       => $entry['id'],
            'version'        => '3',
            'server_url'     => get_bloginfo( 'url' ) . '/?page=gf_liqpay_ipn',
            'result_url'     => $return_url,
        ));
        $url = $html['url']."?data=".$html['data']."&signature=".$html['signature'];


        $url = gf_apply_filters( 'gform_liqpay_request', $form['id'], $url, $form, $entry, $feed, $submission_data );

        //add the bn code (build notation code)
        $url .= '&bn=Rocketgenius_SP';

        $this->log_debug( __METHOD__ . "(): Sending to LiqPay: {$url}" );
                ...
                if ( ! empty( $options ) ) {
                    if ( is_array( $options ) ) {
                        ...
                        foreach ( $options as $option ) {
                            // Trim option label to prevent LiqPay displaying an error instead of the cart.
    ...
    public function get_donation_query_string( $submission_data, $entry_id ) {
        ...
        //truncating to maximum length allowed by LiqPay
    ...
    public function return_url( $form_id, $lead_id ) {
        ...
        $server_port = apply_filters( 'gform_liqpay_return_url_port', $_SERVER['SERVER_PORT'] );
        ...
        $url = add_query_arg( 'gf_liqpay_return', base64_encode( $ids_query ), $pageURL );

        $query = 'gf_liqpay_return=' . base64_encode( $ids_query );
        /**
         * Filters LiqPay's return URL, which is the URL that users will be sent to after completing the payment on LiqPay's site.
         ...
         */
        return apply_filters( 'gform_liqpay_return_url', $url, $form_id, $lead_id, $query  );
    ...
    public function convert_interval( $interval, $to_type ) {
        //convert single character into long text for new feed settings or convert long text into single character for sending to liqpay
    ...
    //------- PROCESSING LIQPAY IPN (Callback) -----------//
...
//        $this->log_debug( __METHOD__ . '(): IPN request received. Starting to process => ' . print_r( $_POST, true ) );
        ...
        $responseLiqPay = json_decode(base64_decode($_POST['data'], true),true);
        $entry = $this->get_entry( $responseLiqPay['order_id'] );
        ...
        if($feed['meta']['mode'] == 'production')
        {
            $private_key_lp = $settings["lp_prod_secret_key"];
        }
        if($feed['meta']['mode'] == 'test')
        {
            $private_key_lp = $settings["lp_test_secret_key"];
        }
...
//            $this->log_debug( __METHOD__ . '(): '. $private_key_lp );
            $this->log_error( __METHOD__ . '(): request could not be verified by LiqPay. Aborting.' );
            return false;
        }
//        $this->log_debug("DEBUG RESPONSE: ".print_r(base64_decode($_POST['data']),true));
//        $responseLiqPay = json_decode(base64_decode($_POST['data'], true),true);
...
//        $feed = $this->get_payment_feed( $entry );
        ...
        //Ignore IPN messages from forms that are no longer configured with the LiqPay add-on
        if ( ! $feed || ! rgar( $feed, 'is_active' ) ) {
            $this->log_error( __METHOD__ . "(): Form no longer is configured with LiqPay Addon. Form ID: {$entry['form_id']}. Aborting." );

            return false;
        }
        $this->log_debug( __METHOD__ . "(): Form {$entry['form_id']} is properly configured." );

        $this->log_debug(__METHOD__. "(): Response LiqPay: ". print_r($responseLiqPay, true));
...
//        }
        ...
        $action = $this->process_ipn( $feed, $entry, $responseLiqPay['status'], $responseLiqPay['paytype'], $responseLiqPay['transaction_id'], rgpost( 'parent_txn_id' ), rgpost( 'subscr_id' ), $responseLiqPay['amount'], rgpost( 'pending_reason' ), rgpost( 'reason_code' ), rgpost( 'mc_amount3' ) );
...
//        }
    ...
    public function get_payment_feed( $entry, $form = false ) {
        ...
        if ( empty( $feed ) && ! empty( $entry['id'] ) ) {
            ...
            $feed = $this->get_liqpay_feed_by_entry( $entry['id'] );
        }

        $feed = apply_filters( 'gform_liqpay_get_payment_feed', $feed, $entry, $form ? $form : GFAPI::get_form( $entry['form_id'] ) );

        return $feed;
    }

    private function get_liqpay_feed_by_entry( $entry_id ) {

        $feed_id = gform_get_meta( $entry_id, 'liqpay_feed_id' );
        $feed    = $this->get_feed( $feed_id );
    ...
    public function post_callback( $callback_action, $callback_result ) {
        ...
        $this->log_debug(__METHOD__." (): CALLBACK ACTION". print_r($callback_action, true));
        ...
        //run gform_liqpay_fulfillment only in certain conditions
...
//            } else {
//                $this->log_debug( __METHOD__ . '(): Entry is already fulfilled or not ready to be fulfilled, not running gform_liqpay_fulfillment hook.' );
//            }
//        }
        ...
        do_action( 'gform_liqpay_ipn_' . $txn_type, $entry, $feed, $status, $txn_type, $transaction_id, $parent_txn_id, $subscriber_id, $amount, $pending_reason, $reason );
        if ( has_filter( 'gform_liqpay_ipn_' . $txn_type ) ) {
            $this->log_debug( __METHOD__ . "(): Executing functions hooked to gform_liqpay_ipn_{$txn_type}." );
        }

        do_action( 'gform_liqpay_post_ipn', $_POST, $entry, $feed, false );
        if ( has_filter( 'gform_liqpay_post_ipn' ) ) {
            $this->log_debug( __METHOD__ . '(): Executing functions hooked to gform_liqpay_post_ipn.' );
        }
    }

    private function verify_liqpay_ipn() {
        ...
        $url = rgpost( 'test_ipn' ) ? $this->sandbox_url : apply_filters( 'gform_liqpay_ipn_url', $this->production_url );

        $this->log_debug( __METHOD__ . "(): Sending IPN request to LiqPay for validation. URL: $url - Data: $req" );

        $url_info = parse_url( $url );

        //Post back to LiqPay system to validate
        $request  = new WP_Http();
        $headers  = array( 'Host' => $url_info['host'] );
        $sslverify = (bool) get_option( 'gform_liqpay_sslverify' );
         ...
         */
        $sslverify = apply_filters( 'gform_liqpay_sslverify', $sslverify );
        ...
        switch ( strtolower( $transaction_type ) ) {
            ...
            case 'subscr_signup' :
                ...
                if ( ! $this->is_valid_initial_payment_amount( $entry['id'], $recurring_amount ) ){
                    ...
                    GFPaymentAddOn::add_note( $entry['id'], sprintf( __( 'Payment amount (%s) does not match subscription amount. Subscription will not be activated. Transaction ID: %s', 'gravityformsliqpay' ), GFCommon::to_money( $recurring_amount, $entry['currency'] ), $subscriber_id ) );
                ...
                switch ( strtolower( $status ) ) {
                    case 'success' :
                        ...
                        if ( ! $this->is_valid_initial_payment_amount( $entry['id'], $amount ) ){
                            ...
                            GFPaymentAddOn::add_note( $entry['id'], sprintf( __( 'Payment amount (%s) does not match product price. Entry will not be marked as Approved. Transaction ID: %s', 'gravityformsliqpay' ), GFCommon::to_money( $amount, $entry['currency'] ), $transaction_id ) );
                    ...
                    case 'reversed' :
                        ...
                        GFPaymentAddOn::add_note( $entry['id'], sprintf( __( 'Payment has been reversed. Transaction ID: %s. Reason: %s', 'gravityformsliqpay' ), $transaction_id, $this->get_reason( $reason ) ) );
                    ...
                    case 'canceled_reversal' :
                        ...
                        GFPaymentAddOn::add_note( $entry['id'], sprintf( __( 'Payment reversal has been canceled and the funds have been transferred to your account. Transaction ID: %s', 'gravityformsliqpay' ), $entry['transaction_id'] ) );
                    ...
                    case 'wait_accept' :
                        ...
                        $action['note']           = sprintf( __( 'Payment is pending. Amount: %s. Transaction ID: %s', 'gravityformsliqpay' ), $amount_formatted, $action['transaction_id']);
    ...
    public function get_entry( $custom_field ) {
        ...
        $hash_matches = apply_filters( 'gform_liqpay_hash_matches', $hash_matches, $entry_id, $hash, $custom_field );
...
//        $this->log_debug( __METHOD__ . "(): IPN message has a valid custom field: {$custom_field}" );
    ...
    public function can_process_ipn( $feed, $entry ) {
        ...
        //Only process test messages coming fron SandBox and only process production messages coming from production LiqPay
        if ( ( $feed['meta']['mode'] == 'test' && ! rgpost( 'test_ipn' ) ) || ( $feed['meta']['mode'] == 'production' && rgpost( 'test_ipn' ) ) ) {
            $this->log_error( __METHOD__ . "(): Invalid test/production mode. IPN message mode (test/production) does not match mode configured in the LiqPay feed. Configured Mode: {$feed['meta']['mode']}. IPN test mode: " . rgpost( 'test_ipn' ) );

            return false;
        }

        /**
         * Filter through your Liqpay business email (Checks to make sure it matches)
         *
         * @param string $feed['meta']['liqpayEmail'] The Liqpay Email to filter through (Taken from the feed object under feed meta)
         ...
         */
        $business_email = apply_filters( 'gform_liqpay_business_email', $feed['meta']['liqpayEmail'], $feed, $entry );
        ...
        if ( strtolower( trim( $recipient_email ) ) != strtolower( trim( $business_email ) ) ) {
            $this->log_error( __METHOD__ . '(): LiqPay email does not match. Email entered on LiqPay feed:' . strtolower( trim( $business_email ) ) . ' - Email from IPN message: ' . $recipient_email );

            return false;
        }

        //Pre IPN processing filter. Allows users to cancel IPN processing
        $cancel = apply_filters( 'gform_liqpay_pre_ipn', false, $_POST, $entry, $feed );

        if ( $cancel ) {
            $this->log_debug( __METHOD__ . '(): IPN processing cancelled by the gform_liqpay_pre_ipn filter. Aborting.' );
            do_action( 'gform_liqpay_post_ipn', $_POST, $entry, $feed, true );
            if ( has_filter( 'gform_liqpay_post_ipn' ) ) {
                $this->log_debug( __METHOD__ . '(): Executing functions hooked to gform_liqpay_post_ipn.' );
            }
    ...
    private function get_reason( $code ) {
        ...
        switch ( strtolower( $code ) ) {
            case 'adjustment_reversal':
                return esc_html__( 'Reversal of an adjustment', 'gravityformsliqpay' );
            case 'buyer-complaint':
                return esc_html__( 'A reversal has occurred on this transaction due to a complaint about the transaction from your customer.', 'gravityformsliqpay' );

            case 'chargeback':
                return esc_html__( 'A reversal has occurred on this transaction due to a chargeback by your customer.', 'gravityformsliqpay' );

            case 'chargeback_reimbursement':
                return esc_html__( 'Reimbursement for a chargeback.', 'gravityformsliqpay' );

            case 'chargeback_settlement':
                return esc_html__( 'Settlement of a chargeback.', 'gravityformsliqpay' );

            case 'guarantee':
                return esc_html__( 'A reversal has occurred on this transaction due to your customer triggering a money-back guarantee.', 'gravityformsliqpay' );

            case 'other':
                return esc_html__( 'Non-specified reason.', 'gravityformsliqpay' );

            case 'refund':
                return esc_html__( 'A reversal has occurred on this transaction because you have given the customer a refund.', 'gravityformsliqpay' );

            default:
                return empty( $code ) ? esc_html__( 'Reason has not been specified. For more information, contact LiqPay Customer Service.', 'gravityformsliqpay' ) : $code;
        }
    }

    public function is_callback_valid() {
        if ( rgget( 'page' ) != 'gf_liqpay_ipn' ) {
            return false;
        }
    ...
    private function get_pending_reason( $code ) {
        switch ( strtolower( $code ) ) {
            case 'address':
                return esc_html__( 'The payment is pending because your customer did not include a confirmed shipping address and your Payment Receiving Preferences are set to allow you to manually accept or deny each of these payments. To change your preference, go to the Preferences section of your Profile.', 'gravityformsliqpay' );

            case 'authorization':
                return esc_html__( 'You set the payment action to Authorization and have not yet captured funds.', 'gravityformsliqpay' );

            case 'echeck':
                return esc_html__( 'The payment is pending because it was made by an eCheck that has not yet cleared.', 'gravityformsliqpay' );

            case 'intl':
                return esc_html__( 'The payment is pending because you hold a non-U.S. account and do not have a withdrawal mechanism. You must manually accept or deny this payment from your Account Overview.', 'gravityformsliqpay' );

            case 'multi-currency':
                return esc_html__( 'You do not have a balance in the currency sent, and you do not have your Payment Receiving Preferences set to automatically convert and accept this payment. You must manually accept or deny this payment.', 'gravityformsliqpay' );

            case 'order':
                return esc_html__( 'You set the payment action to Order and have not yet captured funds.', 'gravityformsliqpay' );

            case 'paymentreview':
                return esc_html__( 'The payment is pending while it is being reviewed by LiqPay for risk.', 'gravityformsliqpay' );

            case 'unilateral':
                return esc_html__( 'The payment is pending because it was made to an email address that is not yet registered or confirmed.', 'gravityformsliqpay' );

            case 'upgrade':
                return esc_html__( 'The payment is pending because it was made via credit card and you must upgrade your account to Business or Premier status in order to receive the funds. Upgrade can also mean that you have reached the monthly limit for transactions on your account.', 'gravityformsliqpay' );

            case 'verify':
                return esc_html__( 'The payment is pending because you are not yet verified. You must verify your account before you can accept this payment.', 'gravityformsliqpay' );

            case 'other':
                return esc_html__( 'Reason has not been specified. For more information, contact LiqPay Customer Service.', 'gravityformsliqpay' );
    ...
    public function init_admin() {
        ...
        //checking if webserver is compatible with LiqPay SSL certificate
//        add_action( 'admin_notices', array( $this, 'check_ipn_request' ) );
    ...
    public function supported_notification_events( $form ) {
        ...
        return array(
            'complete_payment'          => esc_html__( 'Payment Completed', 'gravityformsliqpay' ),
            'refund_payment'            => esc_html__( 'Payment Refunded', 'gravityformsliqpay' ),
            'fail_payment'              => esc_html__( 'Payment Failed', 'gravityformsliqpay' ),
            'add_pending_payment'       => esc_html__( 'Payment Pending', 'gravityformsliqpay' ),
            'void_authorization'        => esc_html__( 'Authorization Voided', 'gravityformsliqpay' ),
            'create_subscription'       => esc_html__( 'Subscription Created', 'gravityformsliqpay' ),
            'cancel_subscription'       => esc_html__( 'Subscription Canceled', 'gravityformsliqpay' ),
            'expire_subscription'       => esc_html__( 'Subscription Expired', 'gravityformsliqpay' ),
            'add_subscription_payment'  => esc_html__( 'Subscription Payment Added', 'gravityformsliqpay' ),
            'fail_subscription_payment' => esc_html__( 'Subscription Payment Failed', 'gravityformsliqpay' ),
        );
    }
    ...
    public function admin_edit_payment_status( $payment_status, $form, $entry ) {
        ...
        $payment_string = gform_tooltip( 'liqpay_edit_payment_status', '', true );
    ...
    public function admin_edit_payment_transaction_id( $transaction_id, $form, $entry ) {
        ...
        $input = '<input type="text" id="liqpay_transaction_id" name="liqpay_transaction_id" value="' . $transaction_id . '">';
    ...
    public function admin_update_payment( $form, $entry_id ) {
        ...
        $payment_transaction = rgpost( 'liqpay_transaction_id' );
        ...
        GFFormsModel::add_note( $entry['id'], $user_id, $user_name, sprintf( esc_html__( 'Payment information was manually updated. Status: %s. Amount: %s. Transaction ID: %s. Date: %s', 'gravityformsliqpay' ), $entry['payment_status'], GFCommon::to_money( $entry['payment_amount'], $entry['currency'] ), $payment_transaction, $entry['payment_date'] ) );
    }
    ...
    public function fulfill_order( &$entry, $transaction_id, $amount, $feed = null ) {
        ...
        do_action( 'gform_liqpay_fulfillment', $entry, $feed, $transaction_id, $amount );
        if ( has_filter( 'gform_liqpay_fulfillment' ) ) {
            $this->log_debug( __METHOD__ . '(): Executing functions hooked to gform_liqpay_fulfillment.' );
        }
    ...
    private function is_valid_initial_payment_amount( $entry_id, $amount_paid ) {
        ...
        //get amount initially sent to liqpay
    ...
    public function liqpay_fulfillment( $entry, $liqpay_config, $transaction_id, $amount ) {
        //no need to do anything for liqpay when it runs this function, ignore
        return false;
    }

    /**
     * Editing of the payment details should only be possible if the entry was processed by LiqPay, if the payment status is Pending or Processing, and the transaction was not a subscription.
     *
     ...
     *
     * Transform data when upgrading from legacy liqpay.
     *
    ...
    public function upgrade( $previous_version ) {
        ...
        if ( empty( $previous_version ) ) {
            $previous_version = get_option( 'gf_liqpay_version' );
        }

        if ( empty( $previous_version ) ) {
            update_option( 'gform_liqpay_sslverify', true );
        }
        ...
        if ( $previous_is_pre_addon_framework ) {
            ...
            //copy existing liqpay transactions to new table
            $this->copy_transactions();

            //updating payment_gateway entry meta to 'gravityformsliqpay' from 'liqpay'
        ...
        if ( ! empty( $previous_version ) && version_compare( $previous_version, '3.2', '<' ) ) {
            delete_transient( 'gravityformsliqpay_tlstest_response' );
        }
    ...
    public function uninstall(){
        parent::uninstall();
        delete_option( 'gform_liqpay_sslverify' );
    }
    ...
    public function update_feed_id( $old_feed_id, $new_feed_id ) {
        ...
        $sql = $wpdb->prepare( "UPDATE {$entry_meta_table} SET meta_value=%s WHERE meta_key='liqpay_feed_id' AND meta_value=%s", $new_feed_id, $old_feed_id );
        $wpdb->query( $sql );
    }
    ...
    public function add_legacy_meta( $new_meta, $old_feed ) {
        ...
        $known_meta_keys = array(
            ...
            'update_post_action', 'delay_notifications', 'selected_notifications', 'liqpay_conditional_enabled', 'liqpay_conditional_field_id',
            'liqpay_conditional_operator', 'liqpay_conditional_value', 'customer_fields',
        );
    ...
    public function update_payment_gateway() {
        global $wpdb;
        $entry_meta_table = self::get_entry_meta_table_name();
        $sql = $wpdb->prepare( "UPDATE {$entry_meta_table} SET meta_value=%s WHERE meta_key='payment_gateway' AND meta_value='liqpay'", $this->_slug );
        $wpdb->query( $sql );
    }
    ...
    public function update_lead() {
        ...
        $sql = $wpdb->prepare(
            "UPDATE {$entry_table}
			 SET payment_status='Paid', payment_method='LiqPay'
    ...
    public function copy_settings() {
        //copy plugin settings
        $old_settings = get_option( 'gf_liqpay_configured' );
        $new_settings = array( 'gf_liqpay_configured' => $old_settings );
    ...
    public function copy_feeds() {
        ...
        if ( $old_feeds ) {
            ...
            foreach ( $old_feeds as $old_feed ) {
                ...
                $new_meta = array(
                    'feedName'                     => $feed_name,
                    'liqpayEmail'                  => rgar( $old_feed['meta'], 'email' ),
                ...
                $conditional_enabled = rgar( $old_feed['meta'], 'liqpay_conditional_enabled' );
                    ...
                    $new_meta['feed_condition_conditional_logic_object'] = array(
                        'conditionalLogic' =>
                            array(
                                ...
                                'rules'      => array(
                                    array(
                                        'fieldId'  => rgar( $old_feed['meta'], 'liqpay_conditional_field_id' ),
                                        'operator' => rgar( $old_feed['meta'], 'liqpay_conditional_operator' ),
                                        'value'    => rgar( $old_feed['meta'], 'liqpay_conditional_value' )
                                    ),
                                )
                            )
                    );
    ...
    public function copy_transactions() {
        //copy transactions from the liqpay transaction table to the add payment transaction table
        global $wpdb;
        $old_table_name = $this->get_old_transaction_table_name();
        if ( ! $this->table_exists( $old_table_name ) ) {
            return false;
        }
        $this->log_debug( __METHOD__ . '(): Copying old LiqPay transactions into new table structure.' );
    ...
    public function get_old_transaction_table_name() {
        global $wpdb;
        return $wpdb->prefix . 'rg_liqpay_transaction';
    }
    ...
    public function get_old_feeds() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'rg_liqpay';
    ...
    public static function get_config_by_entry( $entry ) {
        ...
        $liqpay = GFLiqPay::get_instance();

        $feed = $liqpay->get_payment_feed( $entry );

        if ( empty( $feed ) ) {
            return false;
        }

        return $feed['addon_slug'] == $liqpay->_slug ? $feed : false;
    }
